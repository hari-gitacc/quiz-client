...
const TimerProvider = ({ children, initialTime, onTimeUp }: TimerProviderProps) => {
  ...
  const circumference = 2 * Math.PI * 45; // For circle progress
        ...
        <div className="relative w-24 h-24">
          <svg className="w-full h-full" viewBox="0 0 100 100">
            ...
            <circle
              ...
              strokeDashoffset={circumference - (timeLeft / initialTime) * circumference}
              strokeLinecap="round"
              stroke="currentColor"
              fill="transparent"
              r="45"
...
console.log('Participant count:', data.count);
  ...
  const ParticipantListDisplay = () => (
    <div className="mt-4">
      <h3 className="text-lg font-semibold mb-2">Participants ({participants})</h3>
      {participantList.length > 0 ? (
        <ul className="max-h-60 overflow-y-auto border rounded p-2">
          {participantList.map(participant => (
            <li key={participant.user_id} className="py-1 border-b last:border-b-0">
              <span className="font-medium text-indigo-600">{participant.username}</span>
